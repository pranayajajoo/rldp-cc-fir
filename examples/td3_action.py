...
from metamotivo.td3_action import FBAgent, FBAgentConfig
...
def create_agent(
    ...
    agent_config.model.archi.q.residual_hidden_layers = 2
    # optim
...
class Workspace:
    ...
    def train_offline(self) -> None:
        if self.cfg.fb_type=='online':
            ...
            self.relabel_rewards(self.cfg.task_name, data)
    ...
    def relabel_rewards(self, task, data) -> torch.Tensor:
        data['next_observation'] = data['next']['observation']
        data["terminated"] = np.zeros((data["observation"].shape[0],1), dtype=np.bool)
        set_seed_everywhere(0)
        env = dmc.make(f"{self.cfg.domain_name}_{task}")
        rewards = []
        for i in range(len(data['observation'])):
            with env._physics.reset_context():
                env._physics.set_state(data["next"]["physics"][i])
                env._physics.set_control(data["action"][i])
        ...
        data["reward"] = np.array(rewards).reshape(-1, 1)
