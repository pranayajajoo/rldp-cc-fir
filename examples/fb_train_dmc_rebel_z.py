...
from metamotivo.fb_rebel_z   import FBAgent, FBAgentConfig
...
class Workspace:
    ...
    def train_online(self) -> None:
        ...
        for t in tqdm(range(0, int(self.cfg.num_train_steps))):
            ...
            if t>0 and (t)%self.cfg.horizon == 0:
                obs = torch.tensor(
                ...
                # action = self.agent.act(obs=obs, mean=True)
            ...
            metrics = None
            if t>0 and t%(self.cfg.horizon*self.cfg.num_zs) == 0:
                # import ipdb;ipdb.set_trace()
                metrics = self.agent.update(self.replay_buffer, zs, returns)
                # import ipdb;ipdb.set_trace()
                log_probs = []
                returns = []
                zs = []
