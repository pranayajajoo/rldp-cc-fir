...
from metamotivo.fb_residual_action import FBAgent, FBAgentConfig
...
def create_agent(
    ...
    if residual_critic:
        agent_config.model.archi.q.residual_hidden_dim = 64 #64
    else:
...
class Workspace:
    ...
    def train_online(self) -> None:
        ...
        self.replay_buffer = {"train": DictBuffer(capacity=2_000_000, device=self.agent.device)}
        ...
        for t in tqdm(range(0, int(self.cfg.num_train_steps))):
            ...
            with torch.no_grad(), eval_mode(self.agent._model):
                ...
                action = self.agent.act(obs=obs,z=self.agent.z_inf, mean=False).cpu().numpy()
            ...
            for ep in range(num_ep):
                ...
                while not time_step.last():
                    with torch.no_grad(), eval_mode(self.agent._model):
                        ...
                        action = self.agent.act(obs=obs,z = self.agent.z_inf, mean=True).cpu().numpy()
