...
from metamotivo.fb_rloo_action   import FBAgent, FBAgentConfig
    ...
    def train_online(self) -> None:
        ...
        for t in tqdm(range(0, int(self.cfg.num_train_steps))):
            ...
            if t>self.cfg.warm_start_timesteps and (t)%self.cfg.horizon == 0:
                ...
                action = self.agent._model.actor(obs,z_inf,0.1).mean
                Fs = self.agent._model._forward_map(obs, z_inf, action)
            ...
            if t>self.cfg.warm_start_timesteps and t%(self.cfg.horizon*self.cfg.num_zs) == 0:
                ...
                metrics = self.agent.update(states_buffer, actions_buffer, z_inf, returns, num_trajs_per_state = self.cfg.num_trajs_per_state)
                returns = []
                states_buffer = [[]]
                actions_buffer = [[]]
            ...
            with torch.no_grad(), eval_mode(self.agent._model):
                ...
                action_tensor = self.agent._model.actor(obs,z_inf,0.1).sample()
                action = action_tensor.cpu().numpy()
                
                states_buffer[-1].append(obs)
                actions_buffer[-1].append(action_tensor)
