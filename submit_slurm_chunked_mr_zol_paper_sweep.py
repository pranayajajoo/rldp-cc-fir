...
OUTPUT_DIR_BASE = (
    "/home/pranayaj/projects/def-whitem/pranayaj/results/mrzsrl/metamotivo/results/"
    "ICLR_Seeds/mr_train_dmc_zol_paper_sweep"
)

# DMC ZOL sweep from the paper/code: 2 * 2 * 3 * 3 * 2 * 1 = 72 configs.
RUN_ARGS = {
    "--seed": [1],
    ...
    "--zol_steps": [100, 200],
    "--zol_n_mu": [256],
    "--zol_chi2_coef": [0.0, 0.001, 0.005],
    "--zol_trust_l2_coef": [0.0, 0.02, 0.05],
    "--zol_weight_clip": [50.0, 100.0],
    "--zol_weight_temp": [2.0],
...
def main():
    ...
    configs = list(generate_combinations(RUN_ARGS))
    jobs = [(domain, args) for args in configs for domain in DOMAINS]
    chunks = list(chunk_list(jobs, jobs_per_node))
    print(f"Total ZOL configs: {len(configs)}")
