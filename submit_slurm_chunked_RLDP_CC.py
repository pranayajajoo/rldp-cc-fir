...
def submit_chunk(chunk, sbatch_opts, chunk_idx):
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False) as tmp_script:
        ...
        tmp_script.write("\n")
        tmp_script.write(f"{shlex.quote(sbatch_opts['python_executable'])} -c \"import torch; p=torch.cuda.get_device_properties(0); print('GPU:', torch.cuda.get_device_name(0)); print('VRAM(GB):', p.total_memory/1024**3)\"\n")
        tmp_script.write("\n")
