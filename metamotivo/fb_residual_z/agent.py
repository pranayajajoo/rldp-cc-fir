...
class FBAgent:
    ...
    def update(self, replay_buffer, step: int, actor_update_freq=1) -> Dict[str, torch.Tensor]:
        self.num_updates+=1
    ...
    def update_td3(
        ...
        with torch.no_grad():
            ...
            if self.cfg.train.q_pi_z:
                z_q = self._model.hierarchical_actor(obs)
            else:
                z_q = self.z_inf_rep
            if self.cfg.train.residual_critic:
                target_Fs = self._model._forward_map(next_obs, z_q, next_action)
                target_residual_Qs = self._model._target_qs(next_obs, next_action)
                if self.cfg.train.q_pi_z:
                    next_Qs = (target_Fs * z_q*self.scale).sum(dim=-1) + target_residual_Qs.sum(dim=-1)  # num_parallel x batch
                else:
                    next_Qs = (target_Fs * self.unnorm_z_inf_rep).sum(dim=-1) + target_residual_Qs.sum(dim=-1)  # num_parallel x batch
            else:
                next_Qs = self._model._target_qs(next_obs, next_action).sum(-1)
        ...
        if self.cfg.train.residual_critic:
            with torch.no_grad():
                Fs = self._model._forward_map(obs, z_q, action)  # num_parallel x batch x z_dim
            
            residual_Qs = self._model._qs(obs, action).sum(-1)
            if self.cfg.train.q_pi_z:
                Qs = (Fs * z_q* self.scale).sum(dim=-1) + residual_Qs
            else:
                Qs = residual_Qs + (Fs * self.unnorm_z_inf_rep).sum(dim=-1)  # num_parallel x batch
        else:
        ...
        if self.cfg.train.residual_critic:
            with torch.no_grad():
                output_metrics = {
                    "target_M": target_Q.mean(),
                    "Q1": Qs[0].mean(),
                    "residual_Q1": residual_Qs[0].mean(),
                    "F1": Fs[0].mean(),
                    "q_loss": q_loss,
                }
        else:
    ...
    def update_td3_actor(self, obs: torch.Tensor, clip_grad_norm: float | None) -> Dict[str, torch.Tensor]:
        ...
        cosine_similarity = F.cosine_similarity(z_actor, self.z_inf, dim=-1)
        mse_distance = F.mse_loss(z_actor, self.z_inf)
        if self.cfg.train.q_pi_z:
            z_q = self._model.hierarchical_actor(obs).detach()
        else:
            z_q = self.z_inf_rep
        action_update_freq = obs.shape[0]//z_q.shape[0]
        z_q = z_q.repeat(action_update_freq,1)
        ...
        if self.cfg.train.residual_critic:
            Fs = self._model._forward_map(obs, z_q, action)  # num_parallel x batch x z_dim
            residual_Qs = self._model._qs(obs, action)
            if self.cfg.train.q_pi_z:
                Qs = (Fs * z_q*self.scale).sum(-1)+residual_Qs.sum(-1)
            else:
                Qs = (Fs * self.unnorm_z_inf_rep.repeat(action_update_freq,1)).sum(-1)+residual_Qs.sum(-1)  # num_parallel x batch
        else:
            Qs = self._model._qs(obs, action).sum(-1)
        ...
        return {"actor_loss": actor_loss.detach(), "q": Q.mean().detach(), "cosine_similarity": cosine_similarity.mean().detach(), "mse_distance": mse_distance.detach()}
