...
class FBAgent:
    def __init__(self, **kwargs):
        ...
        self.sampling_actor = SamplingSeedActor(self.cfg.model.action_dim, self.max_log_seed,self.cfg.train.batch_size).to(self.device)
        # create networks
    ...
    def setup_training(self) -> None:
        ...
        self.forward_psm_optimizer = torch.optim.Adam(
            self._model._forward_psm_map.parameters(),
            lr=self.cfg.train.lr_f,
            capturable=self.cfg.cudagraphs and not self.cfg.compile,
            weight_decay=self.cfg.train.weight_decay,
        )
    ...
    def sample_z_psm(self, size, device: str = "cpu"):
        ...
        binary_array = self.int_to_binary_array(z_np, self.max_log_seed)
    ...
    def update(self, replay_buffer, step: int) -> Dict[str, torch.Tensor]:
        ...
        torch.compiler.cudagraph_mark_step_begin()
        if step<400000:
            z = self.sample_z_psm(self.cfg.train.batch_size, device=self.device)
            metrics = self.update_psm(
                obs=obs,
                action=action,
                discount=discount,
                next_obs=next_obs,
            ...
            with torch.no_grad():
                ...
                _soft_update_params(self._forward_psm_map_paramlist, self._target_forward_psm_map_paramlist, self.cfg.train.fb_target_tau)
    ...
    def update_psm(
        ...
        with torch.no_grad():
            ...
            target_Fs = self._model._target_forward_psm_map(next_obs, z, next_action)  # num_parallel x batch x z_dim
        ...
        # compute FB loss
        Fs = self._model._forward_psm_map(obs, z, action)  # num_parallel x batch x z_dim
        ...
        # optimize FB
        self.forward_psm_optimizer.zero_grad(set_to_none=True)
        ...
        self.forward_psm_optimizer.step()
