...
class FBAgent:
    ...
    def update_td3(
        ...
        with torch.no_grad():
            dist = self._model._actor(next_obs, self._model.cfg.actor_std)
        ...
        q_loss.backward()
        # Calculate Q network gradient norm before clipping
        total_norm = 0.0
        for p in self._model._qs.parameters():
        ...
        with torch.no_grad():
            output_metrics = {
                ...
                "Q1": Qs[0].mean(),
                "q_loss": q_loss,
                "q_grad_norm": torch.tensor(total_norm)
    ...
    def update_td3_actor(self, obs: torch.Tensor, clip_grad_norm: float | None) -> Dict[str, torch.Tensor]:
        ...
        actor_loss.backward()
        # calculate and log gradient norm for self._model._actor
        ...
        return {"actor_loss": actor_loss.detach(), "q": Q.mean().detach(),"actor_grad_norm": torch.tensor(total_norm)}
