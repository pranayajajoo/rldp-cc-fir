...
class FBAgent:
    ...
    def update_td3(
        ...
        with torch.no_grad():
            ...
            next_Qs = (target_Fs * self.unnorm_z_inf_rep).sum(dim=-1)
        ...
        Qs = (Fs * self.unnorm_z_inf_rep).sum(dim=-1)
        ...
        # Compute gradient norm of self._model._forward
        grad_norm = 0
        for param in self._model._forward_map.parameters():
    ...
    def update_td3_actor(self, obs: torch.Tensor, clip_grad_norm: float | None) -> Dict[str, torch.Tensor]:
        ...
        # import ipdb;ipdb.set_trace()
        Fs = self._model._forward_map(obs, z_q, action)
        Qs = (Fs * self.unnorm_z_inf_rep.repeat(action_update_freq,1)).sum(-1)
