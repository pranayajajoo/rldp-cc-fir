...
from ..nn_models import build_backward, build_forward, build_actor, eval_mode, build_simple_critic, build_hierarchical_actor_normal
...
class ArchiConfig:
    z_dim: int = 100
    norm_z: bool = True
    q: QArchiConfig = dataclasses.field(default_factory=QArchiConfig)
...
class FBModel(nn.Module):
    def __init__(self, **kwargs):
        ...
        arch = self.cfg.archi
        self.expl_logstd = self.cfg.expl_logstd
        # import ipdb;ipdb.set_trace()
        # create networks
        # import ipdb;ipdb.set_trace()
        self._qs = build_simple_critic(obs_dim,action_dim,self.cfg.archi.q)
        self._hierarchical_actor = build_hierarchical_actor_normal(arch.z_dim,self.expl_logstd)
    ...
    # @torch.no_grad()
    def qs(self, obs: torch.Tensor, action: torch.Tensor):
        return self._qs(self._normalize(obs), action)

    # @torch.no_grad()
    def hierarchical_actor(self, obs=None):
        return self._hierarchical_actor((obs))
    ...
    def act(self, obs: torch.Tensor,z_inf, mean: bool = True) -> torch.Tensor:
        
        dist = self.actor(obs, z_inf, self.cfg.actor_std)
        return dist.mean
    ...
    def reward_inference_with_projection(self, next_obs: torch.Tensor, reward: torch.Tensor, weight: torch.Tensor | None = None) -> torch.Tensor:
        num_batches = int(np.ceil(next_obs.shape[0] / self.cfg.inference_batch_size))
        z = 0
        wr = reward if weight is None else reward * weight
        counter = 0
        for i in range(num_batches):
            ...
            counter+=B.shape[0]
        # import ipdb;ipdb.set_trace()
        return z/counter, self.project_z(z)
